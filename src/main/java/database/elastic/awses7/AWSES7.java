...
public class AWSES7 extends AbstractBaseRequest implements IDatabase<RestHighLevelClient> {

    private static final Logger LOGGER = LogManager.getLogger(AWSES7.class);

    private static AWSES7 elasticSearch;
    ...
    protected AWSES7(String elasticUrl, String elasticPort) {
        this.elasticUrl = elasticUrl;
        this.elasticPort = elasticPort;
    }

    public static AWSES7 getInstance() {
        if (elasticSearch == null) {
            elasticSearch = new AWSES7(elasticUrl, elasticPort);
        }
    ...
    public RestHighLevelClient connect() {
        LOGGER.info(LogConstants.CONNECT_TO_ELASTIC_SEARCH_7_MESSAGE);
        ...
        LOGGER.info(LogConstants.SUCCESS_CONNECT_TO_ELASTIC_SEARCH_7_MESSAGE);
        return restHighLevelClient;
    }
    ...
    public RestHighLevelClient reconnect(int tryInSeconds) {
        for (int i = 0; i < tryInSeconds; i++) {
            try {
                ...
                LOGGER.info(String.format(LogConstants.TRY_TO_RECONNECT_ELASTIC_SEARCH_7, i + 1));
